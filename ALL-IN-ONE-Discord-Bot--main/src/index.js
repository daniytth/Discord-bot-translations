...
process.on('unhandledRejection', error => {
    ...
    if (error) if (error.length > 950) error = error.slice(0, 950) + '... nézd meg a konzolt.';
    if (error.stack) if (error.stack.length > 950) error.stack = error.stack.slice(0, 950) + '... nézd meg a konzolt.';
    ...
    const embed = new Discord.EmbedBuilder()
        ...
        .addFields([
            {
                name: "Hiba",
                value: error ? Discord.codeBlock(error) : "Nincsen hiba.",
            },
            {
                name: "Stack Hiba",
                value: error.stack ? Discord.codeBlock(error.stack) : "Nincsen stack hiba.",
            }
        ])
    consoleLogs.send({
        username: 'Bot Logok',
        ...
        console.log('Hiba az Unhandled Promise Rejection webhooknak történő elküldése közben')
        console.log(error)
    })
});
...
process.on('warning', warn => {
    ...
    const embed = new Discord.EmbedBuilder()
        .setTitle(`🚨・Új Figyelmeztetés Találva`)
        .addFields([
            {
                name: `Figyelmeztetés`,
                value: `\`\`\`${warn}\`\`\``,
    ...
    warnLogs.send({
        ...
        console.log('Hiba a figyelmeztetés webhooknak történő elküldése közben')
        console.log(warn)
    })
});
