    ...
    public void onEvent(GenericEvent event)
    {
        ...
        else if (event instanceof GuildJoinEvent)
        {
            ...
            for(String term: pruneTerms)
                ...
                {
                    LOG.info("Kilépek a(z) " + guild + " szerverről, a szűrt kifejezés miatt '" + term + "'");
                    guild.leave().queue();
                    break;
                }
        }
        else if (event instanceof ReadyEvent)
        {
            ...
            LOG.info("Shard "+shardinfo+" készen áll.");
            vortex.getLogWebhook().send("\uD83C\uDF00 Shard `"+shardinfo+"` csatlakozott. Szerverek: `" // 🌀
                    +event.getJDA().getGuildCache().size()+"` Felhasználók: `"+event.getJDA().getUserCache().size()+"`");
        }
    }
}
