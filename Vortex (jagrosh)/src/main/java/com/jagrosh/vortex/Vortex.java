...
public class Vortex
{
    ...
    public Vortex() throws Exception
    {
        ...
        CommandClient client = new CommandClientBuilder()
                        ...
                        }, t -> event.replyWarning("A segítség nem lett ki küldve mert blokkolod a Közvetlen Üzeneteket.")))
        ...
        shards = new MultiBotManager.MultiBotManagerBuilder()
                ...
                .setActivity(Activity.playing("Betöltés..."))
                .build();
    ...
    public void leavePointlessGuilds()
    {
        ...
        shards.getShardManagers().stream().flatMap(s -> s.getGuilds().stream()).filter(g -> 
        {
            ...
            if(humancount < 5 || botcount > humancount)
            {
        ...
        {
            OtherUtil.safeDM(g.getOwner()==null ? null : g.getOwner().getUser(), Constants.ERROR + " Bocsi, de a szervered **"
                    + g.getName() + "** nem felel meg a követelmények hogy használd a Vortexet. A követelményeket itt "
                    + "találod: <" + Constants.Wiki.START + ">. \n\n" + Constants.WARNING + "Érdemes megfontolnia egy "
                    + "egy másik bot, amelyet kifejezetten az Önéhez hasonló szerverekhez fejlesztettek ki. A botok nyilvános listáját itt találja: "
