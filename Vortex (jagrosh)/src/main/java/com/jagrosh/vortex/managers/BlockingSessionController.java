...
public class BlockingSessionController extends SessionControllerAdapter
{
    ...
    protected class BlockingQueueWorker extends QueueWorker
    {
        ...
        public void run()
        {
            ...
            {
                JDALogger.getLog(SessionControllerAdapter.class).error("Nem sikerült visszalépni", ex);
            }
            while (!connectQueue.isEmpty())
            {
                ...
                {
                    JDALogger.getLog(SessionControllerAdapter.class).error("A node futtatása sikertelen", e);
                    appendSession(node);
                }
            }
            synchronized (lock)
