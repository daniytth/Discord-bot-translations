...
public class FormatUtil {
    ...
    private final static String MULTIPLE_FOUND = "**Több %s találat \"%s\":**";
    ...
    {
        String out = String.format(MULTIPLE_FOUND, "hang csatornák", query);
    ...
    {
        String out = String.format(MULTIPLE_FOUND, "rangok", query);
        for(int i=0; i<6 && i<list.size(); i++)
            out+="\n - "+list.get(i).getName()+" (ID:"+list.get(i).getId()+")";
        if(list.size()>6)
            out+="\n**És "+(list.size()-6)+" több...**";
    ...
    {
        String out = String.format(MULTIPLE_FOUND, "szöveg csatornák", query);
        for(int i=0; i<6 && i<list.size(); i++)
    ...
    {
        String out = String.format(MULTIPLE_FOUND, "felhasználók", query);
        for(int i=0; i<6 && i<list.size(); i++)
    ...
    {
        String out = String.format(MULTIPLE_FOUND, "tagok", query);
        for(int i=0; i<6 && i<list.size(); i++)
    ...
    public static String secondsToTime(long timeseconds)
    {
        ...
        if(years>0)
        {
            builder.append("**").append(years).append("** évek, ");
            timeseconds = timeseconds % (60*60*24*365);
        }
        int weeks = (int)(timeseconds / (60*60*24*7));
        if(weeks>0)
        {
            builder.append("**").append(weeks).append("** hetek, ");
            timeseconds = timeseconds % (60*60*24*7);
        }
        int days = (int)(timeseconds / (60*60*24));
        if(days>0)
        {
            builder.append("**").append(days).append("** napok, ");
            timeseconds = timeseconds % (60*60*24);
        }
        int hours = (int)(timeseconds / (60*60));
        if(hours>0)
        {
            builder.append("**").append(hours).append("** órák, ");
            timeseconds = timeseconds % (60*60);
        }
        int minutes = (int)(timeseconds / (60));
        if(minutes>0)
        {
            builder.append("**").append(minutes).append("** percek, ");
            timeseconds = timeseconds % (60);
        }
        if(timeseconds>0)
            builder.append("**").append(timeseconds).append("** másodpercek");
        String str = builder.toString();
        if(str.endsWith(", "))
            str = str.substring(0,str.length()-2);
        if(str.isEmpty())
            str="**Nincsen idő megadva**";
        return str;
    }
    ...
    public static String secondsToTimeCompact(long timeseconds)
    {
        ...
        if(years>0)
        {
            builder.append("**").append(years).append("**é ");
            timeseconds = timeseconds % (60*60*24*365);
        }
        int weeks = (int)(timeseconds / (60*60*24*7));
        if(weeks>0)
        {
            builder.append("**").append(weeks).append("**hé ");
            timeseconds = timeseconds % (60*60*24*7);
        }
        int days = (int)(timeseconds / (60*60*24));
        if(days>0)
        {
            builder.append("**").append(days).append("**n ");
            timeseconds = timeseconds % (60*60*24);
        }
        int hours = (int)(timeseconds / (60*60));
        if(hours>0)
        {
            builder.append("**").append(hours).append("**ó ");
            timeseconds = timeseconds % (60*60);
        }
        int minutes = (int)(timeseconds / (60));
        if(minutes>0)
        {
            builder.append("**").append(minutes).append("**p ");
            timeseconds = timeseconds % (60);
        }
        if(timeseconds>0)
            builder.append("**").append(timeseconds).append("**ms");
        String str = builder.toString();
        if(str.endsWith(", "))
            str = str.substring(0,str.length()-2);
        if(str.isEmpty())
    ...
    public static Message formatHelp(CommandEvent event, Vortex vortex)
    {
        ...
        if(event.getArgs().isEmpty())
        {
            ...
            content = event.getClient().getSuccess()+" **"+event.getSelfUser().getName()+"** Parancs Kategóriák:";
        }
        else
        {
            ...
            if(commandsInCategory.isEmpty())
                content = event.getClient().getWarning()+" Nincsen ilyen kategória ezzel a névvel `"+event.getArgs()+"` találva.";
            else
                content = event.getClient().getSuccess()+" **"+event.getSelfUser().getName()+"** "
                        +(commandsInCategory.get(0).getCategory()==null ? "Általános" : commandsInCategory.get(0).getCategory().getName())
                        +" Parancs:";
        }
        
        if(commandsInCategory.isEmpty())
        {
            builder.addField(CMD_EMOJI+" Általános Parancsok", "[**"+event.getClient().getPrefix()+"help general**]("+Constants.Wiki.COMMANDS+"#-general-commands)\n\u200B", false);
        ...
        builder.addField("További Segítség", helpLinks(event), false);
    ...
    public static String helpLinks(CommandEvent event)
    {
        return "\uD83D\uDD17 ["+event.getSelfUser().getName()+" Wiki]("+Constants.Wiki.WIKI_BASE+")\n" // 🔗
                + "<:discord:314003252830011395> [Támogatási Szerver]("+event.getClient().getServerInvite()+")\n"
                +  CMD_EMOJI + " [A Parancsok Teljes Leírása]("+Constants.Wiki.COMMANDS+")\n"
                + "<:patreon:417455429145329665> [Adományozások]("+Constants.DONATION_LINK+")";
    }
}
