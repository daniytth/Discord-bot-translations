...
public class OtherUtil
{
    ...
    public final static boolean dehoist(Member m, char symbol)
    {
        ...
        m.modifyNickname(newname).reason("Leemelés.").queue(s -> {}, f -> {});
        return true;
    }
    ...
    public static String[] readLines(String filename)
    {
        try
        {
            ...
            for(int i=0; i<list.length; i++)
                list[i] = values.get(i);
            LOG.info("Sikeresen olvasva "+list.length+" bejegyzés innen: '"+filename+"'");
            return list;
        }
        catch(Exception ex)
        {
            LOG.error("Nem sikerült olvasni '"+filename+"':"+ ex);
            return new String[0];
        }
    }
