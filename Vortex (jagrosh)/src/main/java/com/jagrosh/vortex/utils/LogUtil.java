...
public class LogUtil
{
    ...
    private final static String MODLOG_CLEAN_FORMAT  = LOG_TIME + MODLOG_CASE + EMOJI + MODERATOR + ACTION + " `%d` üzenetek belül %s\n`[Kritériumok]` %s" + REASON;
    private final static String MODLOG_STRIKE_FORMAT = LOG_TIME + MODLOG_CASE + EMOJI + MODERATOR + " adva `%d` strike-ok `[%d → %d]` felhasználónak" + TARGET_USER + REASON;
    private final static String MODLOG_PARDON_FORMAT = LOG_TIME + MODLOG_CASE + EMOJI + MODERATOR + " visszvona `%d` strike-ok `[%d → %d]` felhasználótól" + TARGET_USER + REASON;
    private final static String MODLOG_RAID_FORMAT   = LOG_TIME + MODLOG_CASE + EMOJI + MODERATOR + " `%s` anti-raid mód" + REASON;
    ...
    {
        return auditReasonFormat(moderator, minutes, "["+oldstrikes+" → "+newstrikes+" strike-ok] "+reason);
    }
